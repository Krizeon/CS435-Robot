...
As far as individual parts go, we only designed two of them from scratch on Adobe Illustrator. These parts are the two-inch wheels, wheel hangers, and distance sensor cover. The distance cover is simply two circles spaced apart, and the . The main chassis, battery pack supports, and axles were cut and glued using other woodshop tools. 
...
Once we got these files compatible and all wires were connected and soldered properly, we created our own [motor controlling drivers](https://github.com/Krizeon/CS435-Robot/blob/master/motorcontrols.py) that calls the functions from motor.py to forward, backward, turn left and turn right functions for our robot. These functions control motors that are connected to the M1 and M2 channels of the Featherwing. 
